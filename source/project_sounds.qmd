...
**Participants:** 
`r study_1_demographics[, .N]` Prolific panelists ($M_{age} =$ `r study_1_demographics[, round(mean(age, na.rm = TRUE), digits = 2)]`, $SD_{age} =$ `r study_1_demographics[, round(sd(age, na.rm = TRUE), digits = 2)]`; `r round(study_1_demographics[, mean(female, na.rm = TRUE)*100])`% female) completed the study. We based our sample size on past work using within−subjects designs and a mouse−tracking task (cite). The study employed a 2 (conflict: low vs. high) × 2 (gender model: male vs. female) × 2 (replicates) within-subjects design.
...
#| echo: false
#| warning: false
stimuli <- data.table::fread(file = "../../preference_expression/studies/price_lists/software/study_1/static/csv/long.csv")
```
...
#| warning: false
#| label: tbl-stimuli
#| tbl-cap: Manipulation of Dicision Difficulty

stimuli[, .(Decision = pair,
            `Quality A` = quality_a,
            `Quality B` = quality_b,
            `Price A` = price_a,
            `Price B` = price_b)] %>%
...
# Number of participants and decisions
num_participants <- 100
num_decisions <- 10

# Generate unique participant labels
participant_labels <- replicate(num_participants, 
                                stringi::stri_rand_strings(1, 24, '[a-f0-9]'))


# Initialize an empty data.table
dt <- data.table(participant_label = character(), decision = integer(), 
                 choice = character(), confidence = numeric())

# Function to generate decision switch point
generate_switch_point <- function() {
  sample(c(3:8), 1, prob = c(0.1, 0.2, 0.3, 0.3, 0.1, 0.1))
}

# Generate data for each participant
for (label in participant_labels) {
  # Generate the switch point
  switch_point <- generate_switch_point()
  
  # Generate 10 decisions where the switch from B to A happens at switch_point
  decisions <- rep("B", num_decisions)
  decisions[(switch_point+1):num_decisions] <- "A"
  
  # Generate confidence levels with the described pattern
  # Confidence increases until the switch, then drops and remains flat
  confidence <- c(seq(-1, -5, length.out = switch_point), 
                  seq(-5, -1, length.out =  switch_point - 1), 
                  rep(x = 0, times = num_decisions - switch_point))[1:num_decisions]
  noise <- c(rnorm(n = switch_point, mean = 0, sd = 0.5), 
             rnorm(n = num_decisions - switch_point, mean = -0.25, sd = 0.5))
  confidence <- confidence + noise
  confidence <- pmin(confidence, 0)  # Ensure values are not positive
  
  # Create a data.table for the current participant
  participant_dt <- data.table(participant_label = label, 
                               decision = 1:num_decisions, 
                               choice = decisions, 
                               confidence = confidence)
  
  # Bind to the main data.table
  dt <- rbind(dt, participant_dt)
}
```

```{r identify_switching_point}
#| echo: false
#| warning: false
dt[, swithing_point := min(str_which(string = choice, pattern = "A")) - 1, by = participant_label]
```

```{r distance_to_switchiing_point}
#| echo: false
#| warning: false
dt[, distance := decision - swithing_point, by = participant_label]
```

As a consequence, we expect the confidence with which consumers make these decisions to exhibit a U-shaped pattern that we illustrate for `r num_participants` simulated participants in @fig-sawtooth.
...
# Plot for a Single Participant
ggplot(data = dt[participant_label == participant_labels[42]],
       mapping = aes(x = distance, 
                     y = confidence)) +
  geom_line() +
  geom_point() +
  labs(title = "Confidence Pattern for a Single Participant",
       x = "Switching Point",
       y = "Confidence") +
...
#| warning: false
#| label: fig-sawtooth
#| fig-cap: "Expected U-Shape Pattern in Choice Confidence"

# Plot for All Participants
ggplot(data = dt, 
       mapping = aes(x = distance, 
                     y = confidence)) +
  geom_line(mapping = aes(group = participant_label), alpha = 0.1, col = c_purple) +
  geom_smooth(method = "loess", formula = "y ~ x",
              col = c_purple, fill = c_purple) +
  labs(#title = "Confidence Patterns for All Participants",
       x = "Switching Point",
...
**Validation:** We tested this prediction in pre-test that was conducted online and where participants entered their preference strength manually using a slider. We recruited participants via Prolific and classified `r long[rational == TRUE, length(unique(participant_code))]` participants as rational because they were only switching once. The resulting pattern, illustrated in @fig-reality, matches the prediction.

```{r}
#| label: fig-reality
#| fig-cap: "Fitted Pattern in Self-Reported Choice Confidence"
#| warning: false
...
# Histogram of Switch Points
switch_points <- sapply(1:num_participants, function(i) {
  dt[participant_label == participant_labels[i] & choice == "A", min(decision)]
})
ggplot(data.table(switch_point = switch_points), aes(x = switch_point)) +
  geom_histogram(binwidth = 1, fill = c_purple, alpha = 0.7) +
  geom_histogram(binwidth = 1, col = c_purple, fill = NA) +
  labs(title = "Distribution of Switching Points",
       x = "Switch Point (Decision Number)",
       y = "Frequency") +
